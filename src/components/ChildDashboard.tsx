...
export default function ChildDashboard() {
    ...
    if (task && !task.completed && user) {
      try {
        ...
        if (response.ok) {
          ...
          setUser((prev) =>
            prev ? { ...prev, points: prev.points + task.points } : null
          );
  ...
  const claimReward = async (rewardId: string) => {
    ...
    if (reward && !reward.claimed && user && user.points >= reward.cost) {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          setRewards(rewards.map((r) => (r.id === rewardId ? data.reward : r)));
          // Actualizar puntos del usuari@
          setUser((prev) =>
            prev ? { ...prev, points: prev.points - reward.cost } : null
          );
  ...
  if (!user) {
    return (
      <div className="min-h-screen  from-pink-50 to-blue-50 flex items-center justify-center">
    ...
    <div className="min-h-screen  from-pink-50 to-blue-50 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-purple-600 mb-2 mt-28">
            🌟 Mis Tareas
          </h1>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Completa tus tareas y gana premios 🎈
          </p>
        </div>
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>
              ¡Completa todas tus tareas para ganar más puntos!
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Usa tus puntos para canjear premios increíbles
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {rewards.length === 0 ? (
                ...
                rewards.map((reward) => (
                  <div
                    key={reward.id}
                    className={`border rounded-lg p-4 ${
                      reward.claimed ? "bg-gray-50 border-gray-200" : "bg-white"
                    }`}
                  >

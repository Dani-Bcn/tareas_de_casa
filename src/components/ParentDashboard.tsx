      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8 mt-20">
          ...
          <p className="text-gray-600">
            Gestiona las tareas y recompensas de tus hij@s
          </p>
        </div>
            ...
            <div className="flex flex-col-reverse md:flex-row items-center justify-center md:justify-between gap-4">
              <div className="flex flex-col justify-center text-center">
                ...
                <CardDescription className="text-center md:text-start">
                  Agrega y gestiona las cuentas de tus hij@s
                </CardDescription>
              </div>
              <Button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Este será el usuari@ para iniciar sesión
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    <p className="text-xs text-gray-500">
                      Dale esta contraseña a tu hij@ para que inicie sesión
                    </p>
                      ...
                      <ul className="space-y-1">
                        <li
                          className={`flex items-center gap-1 ${
                            newChild.password.length >= 6
                              ? "text-green-600"
                              : "text-gray-500"
                          }`}
                        >
                          {newChild.password.length >= 6 ? "✓" : "○"} Mínimo 6
                          caracteres
                        </li>
                        <li
                          className={`flex items-center gap-1 ${
                            /[a-zA-Z]/.test(newChild.password)
                              ? "text-green-600"
                              : "text-gray-500"
                          }`}
                        >
                          {/[a-zA-Z]/.test(newChild.password) ? "✓" : "○"} Al
                          menos una letra
                        </li>
                        <li
                          className={`flex items-center gap-1 ${
                            /\d/.test(newChild.password)
                              ? "text-green-600"
                              : "text-gray-500"
                          }`}
                        >
                          {/\d/.test(newChild.password) ? "✓" : "○"} Al menos un
                          número
                        </li>
                        <li
                          className={`flex items-center gap-1 ${
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Esta fecha se usará para calcular la edad automáticamente
                  </p>
                </div>
            ...
            {children.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">
                  No tienes hij@s agregados todavía
                </p>
                <p className="text-sm text-gray-400 ">
                  Haz clic en "Agregar hij@" para comenzar
                </p>
              </div>
            ) : (
              <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 duration-500 gap-4">
                {children.map((child) => (
                  <Card
                    key={child.id}
                    className="bg-linear-to-t  hover:shadow-xl/30 duration-250 from-green-100 to-slate-100  border-purple-200"
                  >
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-gray-500">
                          Edad: {getChildAge(child)} años
                        </p>
                        <p>Tareas pendientes</p>
                        {tasks.filter(
                          (task) => task.childId === child.id && !task.completed
                        ).length === 0 ? (
                          <p className="text-sm text-gray-500">
                          ...
                          <ul className="list-disc list-inside text-sm text-gray-700">
                            {tasks
                              .filter(
                                (task) =>
                                  task.childId === child.id && !task.completed
                              )
                              .map((task) => (
                                <li key={task.id}>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Asigna nuevas tareas a tus hij@s
                  </CardDescription>
                </div>
                <Button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2 flex gap-2">
                    <Button
                      ...
                      disabled={!selectedChild || !newTask.title}
                    >
                      Crear Tarea
                    </Button>
                    <Button
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto">
                {tasks.length === 0 ? (
                  ...
                  tasks.map((task) => (
                    <div
                      key={task.id}
                      className="border rounded-lg p-4 bg-white bg-linear-to-t from-orange-200   to-slate-100"
                    >
          ...
          <Card>
            <CardContent className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-600 mb-2">
                ¡Comienza Agregando un hij@!
              </h3>
              <p className="text-gray-500">
                Para crear tareas y recompensas, primero necesitas agregar al
                menos un hij@ a tu cuenta.
              </p>
            </CardContent>
          </Card>
        )}
